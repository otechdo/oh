...
pub async fn configure_keyboard(app: &mut Os, expert: bool) -> Result<(), Error> {
    loop {
        #[cfg(feature = "ai")]
        assert!(assistant(
            ...
            "Keyboard layout configuration", expert
        ).await.is_ok());
    ...
    loop {
        ...
        assert!(assistant(
            ...
            "Configure Your Keyboard Model", expert
        ).await.is_ok());
